...
test.describe("User Sign-up and Login", function () {
  ...
  test("should allow a visitor to sign-up, login, and logout", async function ({ page }) {
    ...
    await expect(page.locator('[data-test="user-onboarding-dialog-title"]')).toHaveText("Create Bank Account");
    ...
    expect(await page.locator('[data-test="user-onboarding-dialog-content"]').textContent()).toContain("You're all set!");
    await page.locator('[data-test="user-onboarding-next"]').click();
